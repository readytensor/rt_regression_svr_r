...
This is a dockerized, R-based Support Vector model for regression. The purpose of this project is to provide a functional model that can be used as a template for building your own regression model.

The implementation contains a simple preprocessing pipeline which handles missing data and categorical features. The main prediction model is a Support Vector regressor. The model is implemented in R scripts for training and batch prediction tasks.
...
- Run the script `src/train.r` to train the Support Vector model. This will save the model artifacts, including the preprocessing pipeline, in the path `./model_inputs_outputs/model/artifacts/`.
